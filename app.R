...
RNAseq_data <- read.csv("RNASeq_data.csv", header = TRUE)


control_df <- RNAseq_data %>%
  filter(stage == "Control") %>%
  select(WBID, replicate, control_CPM = CPM)

fc_df <- RNAseq_data %>%
  left_join(control_df, by = c("WBID", "replicate")) %>%
  filter(!is.na(control_CPM) & control_CPM > 0) %>%
  mutate(fc = CPM / control_CPM) %>%
  filter(stage != "Control")

summary_df <- fc_df %>%
  group_by(WBID, gene_name, stage) %>%
  summarise(
    fc_mean = mean(fc),
    .groups = "drop"
  )
